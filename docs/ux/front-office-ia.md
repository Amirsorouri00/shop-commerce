...
| `/settings` | Profile, phone, language, notification prefs | **none — see below** | authed | **N** | Minimal; the home for logout, which has no surface today despite `auth.logout()` existing |

**`/settings` serves no J1–J9 journey — and that is a Phase 3 taxonomy gap, not an oversight here.** Account self-management (view/change your phone, language, notification preferences, and log out) is a real, recurring customer need that no journey in `docs/ux/journey-map.md` covers: J1–J9 model the *ordering* lifecycle plus support, and stop at the account boundary. Two honest options: invent a J10 "manage my account" journey retroactively, or treat `/settings` as a **cross-cutting account-management surface** outside the journey taxonomy. **Recommended: the latter** — journeys model goal-directed flows with entry/intent/terminal states, and settings is a destination people visit incidentally rather than a journey they undertake. Recorded as an explicit exception to the "every screen serves a journey" rule rather than left as a silent orphan. **Confidence: medium** — a reasonable modelling call, but if Phase 6 finds the same shape on the operator side (an internal-user settings surface with no operator journey), that would argue for adding the journey instead. Revisit then; reversal cost is low (documentation only).
