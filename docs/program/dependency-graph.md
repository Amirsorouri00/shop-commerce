...
2. **G-03 was over-serialized.** Chain A placed it after G-05 and G-06; it has no real dependency on either. It is an unauthenticated production money-mover. An environment gate provides immediate **containment**, but the structural fix (authenticated control plane + verified callback ingress) is WP-01/WP-02 and must not be skipped because containment landed — `sandbox.module.ts:188` lacks the `NODE_ENV` guard that `dev-gateway.module.ts:42-44` already has. **Moved to "not gated."**
